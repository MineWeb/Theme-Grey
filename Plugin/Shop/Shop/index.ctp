<div class="container-site">
			...
			<ul class="list-category" role="tablist">
				...
				foreach ($search_categories as $k => $v) {
				  $i++;
				?>
					<li><a href="<?= $this->Html->url(array('controller' => 'c/'.$v['Category']['id'], 'plugin' => 'shop')) ?>" class="<?= (isset($category) AND $v['Category']['id'] == $category OR !isset($category) AND $i == 1) ? ' active' : ''; ?>"><?= before_display($v['Category']['name']) ?></a></li>
				<?php } ?>
			</ul>	
			<hr>
			<?php if(!$isConnected) { ?>
